...
def test_reopening_resets_every_field_including_the_password(page, fresh_user):
    ...
    # The size goes back to the 10 GB default (the markup's value, and the prefill opening the dialog
    # writes) rather than emptying — which is only meaningful because it was changed to 7 above.
    assert page.input_value("#vault-size-gb") == "10"
