...
def test_boot_ddl_adds_the_fk_for_every_model_fk_column_it_introduces():
    ...
    # Pre-existing on main before this guard existed (NOT introduced by the file-ops PR): the boot
    # DDL adds this self-referential column without its FK, exactly the divergence this test catches.
    # Grandfathered so the guard is green here and catches the NEXT one; tracked for a main-focused
    # fix rather than folded into an unrelated feature PR (its own guarded ADD CONSTRAINT, same shape
    # as files.modified_by's).
    KNOWN_PREEXISTING_GAPS = {("temporary_credentials", "created_by_temp_credential_id")}
